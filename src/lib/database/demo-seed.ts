...
export const demoSecondOrganization: Organization = {
  id: 'org_0002',
  name: 'Studio Vitta',
  slug: 'studio-vitta',
  type: 'STUDIO',
...
export const demoBillingSettings: OrganizationBillingSettings = {
...
export const demoPaymentAccount: PaymentAccount = {
  id: 'pacc_0001',
  organizationId: DEMO_ORG_ID,
  provider: 'mock',
...
export const demoSplits: PaymentSplit[] = []
...
for (const student of demoStudents) {
  ...
  for (let offset = -11; offset <= 0; offset += 1) {
    ...
    if (offset === 0) {
      ...
      // Meses recentes ainda podem ter cobrança em aberto.
      pushCharge({
        student,
        membership,
        competence,
        status: rand() < 0.03 ? 'OVERDUE' : 'PAID',
        method,
      })
    } else {
...
export const demoExercises: Exercise[] = EXERCISE_SEEDS.map(([name, muscleGroup, equipment], index) => ({
  id: id('exr', index + 1),
  organizationId: null,
  name,
  muscleGroup,
...
export const demoWorkoutExercises: WorkoutExercise[] = []
export const demoWorkoutAssignments: WorkoutAssignment[] = []
...
export const demoLeads: Lead[] = Array.from({ length: 28 }, (_, index) => {
  const name = `${pick(FIRST_NAMES)} ${pick(LAST_NAMES)}`
  return {
    id: id('lead', index + 1),
    organizationId: DEMO_ORG_ID,
...
// ── Régua de cobrança ────────────────────────────────────────────────────────
export const demoCollectionRules: CollectionRule[] = [
...
export const DEMO_STUDENT_ID = demoStudents[0].id
export const DEMO_OWNER_STAFF_ID = 'staff_0001'
